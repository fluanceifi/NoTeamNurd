...
def _check_h(h: float, hr) -> bool:
    """H(색조) 범위를 확인하는 헬퍼 함수. 튜플과 리스트(겨울톤) 모두 처리"""
    if isinstance(hr, list):
        # [(0, 15), (310, 360)] 같은 리스트 형태 처리
        return any(r[0] <= h <= r[1] for r in hr)
    elif isinstance(hr, tuple):
        # (0, 70) 같은 일반 튜플 처리 (순환은 없다고 가정)
        return hr[0] <= h <= hr[1]
    return False
...
def get_recommended_backgrounds(hsv_data: dict) -> list:
    ...
    tone_palette = {
        # 🌸 봄 (Spring) - 따뜻하고 화사한 뉴트럴 톤 (베이지, 크림, 피치 계열)
        '봄 라이트': [(255, 248, 220), (250, 238, 205), (240, 225, 195)],  # 소프트 베이지, 피치 크림, 라이트 골드 (Image 1 'beige' 느낌)
        '봄 브라이트': [(255, 253, 237), (255, 240, 210), (235, 245, 240)],  # 클리어 크림, 퓨어 피치, 민트 스카이 (밝고 깨끗한 느낌)
        '봄 웜': [(245, 245, 220), (235, 225, 200), (220, 210, 180)],  # 웜 아이보리, 골든 베이지, 샌드 브라운 (Image 2 'cozy' 느낌)

        # 🌊 여름 (Summer) - 차갑고 부드러운 뉴트럴 톤 (블루, 라벤더, 핑크 계열)
        '여름 라이트': [(240, 248, 255), (225, 235, 245), (230, 235, 250)],
        # 페일 블루 화이트, 소프트 스카이, 페일 라벤더 (Image 1 'cloudysky', 'lavender' 느낌)
        '여름 뮤트': [(225, 230, 230), (205, 215, 225), (210, 205, 210)],
        # 소프트 그레이지, 더스티 블루, 모브 그레이 (Image 2 'mystery', 'grey' 느낌)
        '여름 쿨': [(230, 240, 255), (210, 225, 240), (220, 225, 235)],  # 아이시 블루 화이트, 쿨 스카이, 라벤더 그레이 (Image 2 'sky' 느낌)

        # 🍂 가을 (Autumn) - 따뜻하고 차분한 뉴트럴 톤 (브라운, 카키, 토프 계열)
        '가을 뮤트': [(235, 225, 210), (215, 205, 190), (195, 185, 170)],  # 오트밀, 더스티 로즈 브라운, 웜 토프 (Image 2 'cocoa' 느낌)
        '가을 웜': [(243, 233, 210), (225, 210, 185), (200, 180, 155)],
        # 웜 베이지, 캐러멜 베이지, 어시 브라운 (Image 1 'brick'의 부드러운 버전)
        '가을 다크': [(243, 230, 209), (200, 185, 170), (170, 155, 140)],  # 딥 크림, 웜 그레이 브라운, 다크 토프

        # ❄️ 겨울 (Winter) - 매우 깨끗하거나 차가운 톤 (화이트, 쿨 그레이, 아이시 블루 계열)
        '겨울 브라이트': [(255, 255, 255), (235, 245, 255), (220, 230, 245)],
        # 퓨어 화이트, 아이시 스카이, 쿨 페일 블루 (Image 2 'grey'의 쿨한 버전)
        '겨울 딥': [(235, 235, 245), (190, 200, 210), (160, 170, 180)],  # 쿨 라이트 그레이, 미디엄 쿨 그레이, 딥 차콜 그레이
        '겨울 쿨': [(248, 248, 255), (210, 220, 235), (200, 205, 210)],  # 고스트 화이트, 쿨 라벤더 그레이, 미디엄 쿨 블루

        # 🌟 뉴트럴 (Neutral) - 무채색 계열 (가장 무난함)
        '뉴트럴 라이트': [(245, 245, 245), (230, 230, 230), (220, 220, 220)],  # 라이트 그레이, 실버 그레이, 쿨 화이트
        '뉴트럴 미디엄': [(200, 200, 200), (180, 180, 180), (160, 160, 160)],  # 미디엄 그레이, 웜 그레이, 다크 그레이
    }
    ...
    for tone_name, category, hr, sr, vr in tone_table:
        ...
        if score == 3:
            ...
            print(f"✅ 완전 매치: {tone_name}")
            return tone_palette[tone_name]  # <- 1단계의 팔레트 사용

        # 부분 매치 중 최고 점수 기록
        if score > best_score:
            best_score = score
            best_match = tone_name  # <- 톤 이름(string)만 저장

        if best_match and best_score >= 2:
            latest_color_category = best_match
            print(f"🔍 부분 매치 (2/3): {best_match}")
            return tone_palette[best_match]  # <- 1단계의 팔레트 사용

    # 여전히 매치가 안 되면 HSV 값을 기반으로 추론
    print(f"⚠️ 매치 실패 (1/3 이하), HSV 기반 추론...")

    if s <= 0.18:  # 채도가 매우 낮으면 뉴트럴
        if v >= 0.7:
            latest_color_category = '뉴트럴 라이트'
        else:
            latest_color_category = '뉴트럴 미디엄'
    elif 0 <= h <= 90:  # 따뜻한 색조 (봄/가을)
        if v >= 0.7:  # 밝음
            if s >= 0.6:
                latest_color_category = '봄 브라이트'
            else:
                latest_color_category = '봄 라이트'
        else:  # 어두움
            if s >= 0.4:
                latest_color_category = '가을 웜'
            else:
        ...
        if v >= 0.7:  # 밝음
            if s <= 0.35:
                ...
                latest_color_category = '여름 쿨'
        else:  # 어두움
            if s >= 0.5:
                latest_color_category = '겨울 딥'
            else:
                latest_color_category = '여름 뮤트'

    print(f"  -> 추론 결과: {latest_color_category}")
    return tone_palette[latest_color_category]  # <- 1단계 팔레트 사용
...
def apply_outfit_synthesis(person_image_path: str, outfit_type: str) -> str:
    ...
    try:
        ...
        if os.path.exists(result[0]):
            ...
            # --- ✨ 리사이징 코드 시작 ✨ ---
            try:
                # 1. 원본 이미지(person_image_path)를 열어 원본 크기(original_size)를 가져옵니다.
                with Image.open(person_image_path) as original_img:
                    original_size = original_img.size  # (width, height)

                print(f"📏 원본 이미지 크기: {original_size}")

                # 2. 합성된 결과 이미지(result[0])를 엽니다.
                with Image.open(result[0]) as synthesized_img:
                    print(f"📐 합성된 이미지 크기: {synthesized_img.size}")

                    # 3. 원본 크기와 다를 경우, 원본 크기로 리사이징합니다. (고화질 LANCZOS 필터 사용)
                    if synthesized_img.size != original_size:
                        print(f"🔄 이미지를 원본 크기 {original_size}로 리사이징합니다...")
                        resized_img = synthesized_img.resize(original_size, Image.Resampling.LANCZOS)
                    else:
                        resized_img = synthesized_img  # 크기가 같으면 그대로 사용

                    # 4. JPEG로 저장하기 전에 RGB 모드로 변환 (필수)
                    if resized_img.mode != 'RGB':
                        resized_img = resized_img.convert('RGB')

                    # 5. 최종 경로(result_path)에 리사이징된 이미지를 저장합니다.
                    resized_img.save(result_path, 'JPEG', quality=95)

            except Exception as e:
                print(f"❌ 리사이징 중 오류 발생: {e}. 원본 파일로 그냥 복사합니다.")
                shutil.copy2(result[0], result_path)  # 실패 시 비상 로직
