...
# 퍼스널 컬러 옵션 매핑 (각 계절별 3가지 톤 이름)
tone_options = {
    '봄': ['봄 라이트', '봄 브라이트', '봄 웜'],
    '여름': ['여름 라이트', '여름 뮤트', '여름 쿨'],
    '가을': ['가을 뮤트', '가을 웜', '가을 다크'],
    '겨울': ['겨울 브라이트', '겨울 딥', '겨울 쿨'],
}
...
def analyze_face(image_path):
    ...
    skin_mask = (parsed_classes == 1).squeeze().cpu().numpy().astype(bool)
    ...
    skin_pixels = skin_pixels.astype(np.float32) / 255.0
    ...
    return {'h': float(round(mean_h, 1)), 's': float(round(mean_s, 1)), 'v': float(round(mean_v, 1))}
...
def get_recommended_backgrounds(hsv_data: dict) -> list:
    ...
    # 계절 카테고리 판단 및 전역 변수 저장
    for tone_name, category, hr, sr, vr in tone_table:
        if hr[0] <= h <= hr[1] and sr[0] <= s <= sr[1] and vr[0] <= v <= vr[1]:
            latest_color_category = tone_name  # 12개 중 하나의 톤 이름 저장
            return tone_palette[category]     # 배경색은 기존대로 계절 카테고리로 유지
...
latest_paths = []  # 전역 변수
latest_color_category = None  # 전역 변수
...
# ➕ 퍼스널컬러 결과 조회 엔드포인트 추가
@app.route('/personal-color', methods=['GET'])
def get_personal_color():
    if latest_color_category:
        return jsonify({
            'success': True,
            'color': latest_color_category  # 12개 톤 중 하나만 반환
        })
    return jsonify({'success': False, 'message': '분석 결과 없음'}), 404
